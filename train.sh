...
accelerate launch \
     --main_process_ip $MASTER_ADDR --main_process_port $MASTER_PORT --num_cpu_threads_per_process=$SLURM_CPUS_ON_NODE \
     --num_machines $SLURM_NNODES --num_processes $WORLD_SIZE --machine_rank=$SLURM_NODEID \
     mup_demo/train.py "$@"
