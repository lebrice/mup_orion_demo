...
from simple_parsing.helpers import field, choice
...
class Config:
    ...
    model_type: str | None = choice(MODEL_TYPES, default=None)
    ...
    truncated in block of this size for training. Default to the model max input length for 
    ...
    report_to: Literal["tensorboard", "wandb", "comet_ml", "all"] = "all"
    """The integration to report the results and logs to. Supported platforms are
    `"tensorboard"`, `"wandb"` and `"comet_ml"`. Use `"all"` (default) to report to all 
    ...
    def __post_init__(self):
        ...
        if (
            ...
            def _raise_if_not_good_extension(file_path: str, attr_name: str):
                extension = file_path.split(".")[-1]
                if extension not in ["csv", "json", "txt"]:
                    raise ValueError(
                        f"`{attr_name}` should be a csv, json or txt file."
                    )

            if self.train_file is not None:
                _raise_if_not_good_extension(self.train_file, "train_file")
            if self.validation_file is not None:
                _raise_if_not_good_extension(self.validation_file, "validation_file")
        if self.push_to_hub and self.output_dir is None:
            raise ValueError(
...
def main():
        ...
        if config.push_to_hub:
            if config.hub_model_id is None:
                repo_name = get_full_repo_name(
                    Path(config.output_dir).name, token=config.hub_token
                )
            else:
    ...
    if config.dataset_name is not None:
        ...
        if extension == "txt":
            extension = "text"
            dataset_args["keep_linebreaks"] = not config.no_keep_linebreaks
    ...
    if config.config_name:
        ...
        config = CONFIG_MAPPING[config.model_type]()
    ...
    if config.resume_from_checkpoint:
        if (
            ...
            accelerator.print(
                f"Resumed from checkpoint: {config.resume_from_checkpoint}"
            )
    ...
    for epoch in range(starting_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if config.resume_from_checkpoint and epoch == starting_epoch:
        ...
        if config.push_to_hub and epoch < config.num_train_epochs - 1:
            accelerator.wait_for_everyone()
